...
pub fn basic_calendar() -> Calendar {
    let res: Calendar = Calendar {
        weekend: vec![Weekday::Sat, Weekday::Sun].into_iter().collect(),
...
pub fn calendar_unions(calendars: &Vec<Calendar>) -> Calendar {
    ...
    calendars.iter().for_each(|cal| result_cal.union(cal));
...
impl Calendar {
    ...
    pub fn intersection(&mut self, calendar: &Calendar) {
        self.holidays = self
            .holidays
            .intersection(&calendar.holidays)
            .cloned()
            .collect();
        self.weekend = self
            .weekend
            .intersection(&calendar.weekend)
            .cloned()
            .collect();
    }
}
...
mod tests {
    ...
    fn calendar_union_test() {
        let christmas_day = NaiveDate::from_ymd_opt(2023, 12, 25).unwrap();
        let boxing_day = NaiveDate::from_ymd_opt(2023, 12, 26).unwrap();
        let mut cal1: Calendar = c::Calendar {
            weekend: vec![Weekday::Sat].into_iter().collect(),
            holidays: [christmas_day].into_iter().collect(),
        };
        let cal2: Calendar = c::Calendar {
            weekend: vec![Weekday::Sun].into_iter().collect(),
